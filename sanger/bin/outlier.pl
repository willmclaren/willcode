...
if($args{'sims'}) {
	if(!($args{'nht'})) {
		sim(\%hetcont, \%hetcuts, scalar keys %seen_samples, ($args{'sims'} ? $args{'sims'} : 1000), $args{'o'});
	}
	
	if(!($args{'nht'})) {
		sim(\%homcont, \%homcuts, scalar keys %seen_samples, ($args{'sims'} ? $args{'sims'} : 1000), $args{'o'});
	}
}
...
sub sim {
	my ($cont, $cuts, $total_samples, $sim_number, $out_stem) = @_;
