...
for (chrm in c(1,3,20,21,22)) { #loop for chrm BEGIN

print(paste("chrm", chrm))

genotyped <- read.table(paste("Fumi/", target, ".chrm", chrm, ".bgl", sep=""),
                        header=F)
dim(genotyped)

phasedfile <- paste(template, ".", target, "_", chip, "_phase_chr", chrm, ".na.lim.bgl", sep="")

if(!file.exists(phasedfile)) {
	next
}

phased <- read.table(phasedfile, header=F)
dim(phased)
...
write.table(output,
            paste(template, ".", target, "_", chip, "_phase.chrm", chrm, ".all.r2", sep=""),
