...
phased <- read.table(paste("Fumi/", template, ".", target, "_", chip, ".chrm", chrm, ".phased", sep=""),
                        header=F)
dim(phased)
...
write.table(output,
            paste(template, ".", target, "_", chip, ".chrm", chrm, ".beagle.lt0.05.r2", sep=""),
