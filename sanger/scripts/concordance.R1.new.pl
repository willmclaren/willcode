...
frq <- read.table(paste(target, ".frq", sep=""),
                  header=T)
dim(frq)
...
genotyped <- read.table(paste(target, ".chr", chrm, ".bgl", sep=""), header=F)
dim(genotyped)

phased <- read.table(paste(template, ".", target, "_1M.chr", chrm, ".phased", sep=""), header=F)
dim(phased)
...
write.table(output,
            paste(template, ".", target, "_", chip, ".chr", chrm, ".r2", sep=""),
