...
foreach $pair(keys %counts) {
	...
	foreach $geno(keys %{$counts{$pair}}) {
		$freq{$snpa}{$snpb}{$geno} = $counts{$pair}{$geno} / $totals{$pair};
...
while(<>) {
	...
	foreach $a(keys %freq) {
		foreach $b(keys %{$freq{$a}}) {
			...
			elsif(!$counts{$a." ".$b}{$geno{$a}." ".$geno{$b}}) {
				$miss = 0;
				
				foreach $geno(keys %{$counts{$a." ".$b}}) {
					$miss = 1 if $geno =~ /^$geno{$a}/;
					$miss = 1 if $geno =~ /$geno{$b}$/;
				}
				
				print FULL "$sample\t$a\t$b\t$geno{$a}\t$geno{$b}\tMISSING\n" if $miss;
			}
		}
	}
