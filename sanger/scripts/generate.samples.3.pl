...
foreach $num(1..$count) {
	...
	foreach $chrom(1..22) {
		...
		system("perl ~/scripts/convert.pl -l $popa\.$popb\.$num\.f$gen\.chr$chrom\.legend -p $popa\.$popb\.$num\.f$gen\.chr$chrom\.phased -s $popa\.$popb\.$num\.f$gen\.chr$chrom\.sample -o $popa\.$popb\.$num\.f$gen\.chr$chrom");
	}
